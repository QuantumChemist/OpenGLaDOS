    ...
    async def random_message_task(self):
        ...
        if guild:
            ...
            if valid_channels:
                ...
                # Split llm_answer into chunks of up to 1024 characters
                chunks = [
                    llm_answer[i : i + 1024] for i in range(0, len(llm_answer), 1024)
                ]

                # Create the embed
                embed = discord.Embed(
                    title="<a:danger:1338111432214057102> Danger!!!! <a:danger:1338111432214057102>",
                    description="Random danger message from OpenGLaDOS",
                    color=discord.Color.red(),
                )

                # Add each chunk as a separate field
                for idx, chunk in enumerate(chunks):
                    continuation = " " if idx > 0 else ""
                    embed.add_field(
                        name=f"<a:danger:1338111432214057102> Danger!!!! <a:danger:1338111432214057102> {continuation}",
                        value=chunk,
                        inline=False,
                    )

                embed.set_footer(text="This message was generated by OpenGLaDOS. ")
